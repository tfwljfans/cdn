(function($, window, document) {
    ...
    Paging.prototype = {
        ...
        createDom: function() {
            ...
            var firstPage, prePage, nextPage, lastPage, total_1, total_2, jump
            if (that.options.lc === 'us'){
                firstPage = 'First'
                prePage = 'Prev'
                lastPage = 'Last'
                nextPage = 'Next'
            ...
            that.options.jump ? jumpDom = '<input type="text" placeholder="1" class="jump-text" id="jumpText"><button type="button" class="jump-button" id="jumpBtn">'+jump+'</button>' : jumpDom = '';
            content = '<button type="button" id="firstPage" class="turnPage first-page">'+firstPage+'</button>' +
                '<button class="turnPage" id="prePage">'+prePage+'</button>' +
                '<div class="pageWrap" style="width:' + wrapLength + 'px">' +
                '<ul id="pageSelect" style="transition:all ' + that.options.slideSpeed + 'ms">' +
                ulDom +
                '</ul></div>' +
                '<button class="turnPage" id="nextPage">'+nextPage+'</button>' +
                '<button type="button" id="lastPage" class="last-page">'+lastPage+'</button>' +
                jumpDom +
                '<p class="total-pages">'+total_1+'&nbsp;' +
